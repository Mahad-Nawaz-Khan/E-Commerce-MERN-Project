...
function FlashSales() {
  ...
  const visibleProducts = todaysDeals.slice(startIndex, startIndex + productsPerView)
  const canGoPrevious = startIndex > 0
  const canGoNext = startIndex + productsPerView < todaysDeals.length
  ...
  return (
    <section className="w-full px-4 sm:px-6 lg:px-10 xl:px-16 2xl:px-24 py-12 sm:py-16 mt-4 sm:mt-12">
      <div className="flex flex-col gap-7">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0">
              ...
              <div className="flex items-center gap-2 sm:gap-4 text-sm">
                {[
                  ...
                  <div key={label} className="flex items-center gap-3 sm:gap-6">
                    ...
                    {i < arr.length - 1 && <div className="font-bold text-countdown text-xl">:</div>}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-2 sm:gap-4">
            ...
            <Button
              ...
              className="h-8 w-8 sm:h-10 sm:w-10 rounded-full border-0 bg-surface"
                disabled={!canGoNext}
                onClick={() => setStartIndex((index) => Math.min(todaysDeals.length - productsPerView, index + 1))}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-7.5 lg:justify-items-start">
          {visibleProducts.map((product, index) => (
            <div key={product.id} className={index === 3 ? 'md:max-lg:hidden' : ''}>
              <ProductCard product={product} showNewBadge={false} />
            </div>
          ))}
        </div>
        <div className="flex justify-center pt-5"><Link to="/shop"><Button className="h-12 bg-secondary hover:bg-secondary-hover px-12 text-secondary-foreground rounded-sm">View All Products</Button></Link></div>
      </div>
    </section>
  )
}

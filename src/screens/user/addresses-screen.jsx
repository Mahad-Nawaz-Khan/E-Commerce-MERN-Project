...
export function AddressesScreen() {
  ...
  const userId = res?.data?.[0]?.user // not needed; addresses carry their own id
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {addresses.map((addr) => {
            const isDefault = addr.isDefault || addr.id === userId?.defaultAddressId
            return (
              <Card key={addr.id}>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  {!isDefault && <Button variant="ghost" size="sm" onClick={() => makeDefault(addr.id)} loading={settingDefault}><Star className="h-3.5 w-3.5" /> Set default</Button>}
                  <Button variant="ghost" size="sm" className="text-[var(--color-sale)] hover:text-[var(--color-sale)]" onClick={() => setConfirmDelete(addr.id)}><Trash2 className="h-3.5 w-3.5" /> Delete</Button>
                </div>
              </Card>
            )
          })}
